import bcrypt
import psycopg2
from models import UserRole, UserStatus
from utils import register_user_bcrypt
db_params = {
    'database': 'postgres',
    'user': 'postgres',
    'password': '2209',
    'host': 'localhost',
    'port': 5432
}

conn = psycopg2.connect(**db_params)
cursor = conn.cursor()

class Response:
    def __init__(self, data: str, status_code: int):
        self.data = data
        self.status_code = status_code

def hash_password_bcrypt(raw_password):
    salt = bcrypt.gensalt()  
    hashed_password = bcrypt.hashpw(raw_password.encode(), salt)
    return hashed_password.decode(), salt.decode()

def match_password_bcrypt(raw_password, hashed_password, salt):
    hashed_password_stored = bcrypt.hashpw(raw_password.encode(), salt.encode())
    return hashed_password_stored == hashed_password.encode()

def create_table():
    create_user_query = """create table if not exists users(
        id serial PRIMARY KEY,
        username varchar(100) unique not null,
        password varchar(255) not null,
        "role" varchar(100) not null,
        status varchar(100) not null,
        login_try_count int not null default 0
    );
    """
    create_todo_query = """create table if not exists todo(
        id serial primary key,
        name varchar(100) not null,
        description varchar(100),
        todo_type varchar(100) not null,
        user_id int not null references users(id)
    );
    """
    cursor.execute(create_user_query)
    cursor.execute(create_todo_query)

def migrate():
    insert_admin_user_query = """
    insert into users(username, password, "role", status, login_try_count)
    values (%s, %s, %s, %s, %s);
    """
    hashed_password, salt = hash_password_bcrypt('123')  # Hashed password for '123'
    user_data = ('admin', hashed_password, UserRole.ADMIN.value, UserStatus.ACTIVE.value, 0)
    cursor.execute(insert_admin_user_query, user_data)
    conn.commit()

def init():
    create_table()
    migrate()

def commit(func):
    def wrapper(*args, **kwargs):
        result = func(*args, **kwargs)
        conn.commit()
        return result
    return wrapper

if __name__ == '__main__':
    init()

 
    username = 'Avf'
    password = '2209'
    role = 'user'
    status = 'inactive'
    login_try_count = 0
    register_user_bcrypt(username, password, role, status, login_try_count, cursor, conn)

    print('Foydalanuvchi muvaffaqiyatli ro\'yxatdan o\'tdi.')
